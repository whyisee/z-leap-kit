...
app.get("/api/topics/:id", (context) => {
  const lang = getLangFromRequest(context.req.raw);
  const topic = getTopicById(Number(context.req.param("id")), lang);
...
function isTopicType(value: string | undefined): value is TopicType {
  return value === "discussion" || value === "article" || value === "project" || value === "resource" || value === "announcement";
}
