...
- SQLite WAL 适合轻量并发读写场景，但备份和 checkpoint 需要认真处理：https://www.sqlite.org/wal.html
...
SQLite WAL 模式不能只复制单个 `.sqlite` 文件。备份脚本要使用 SQLite 的 `.backup` 或应用层安全备份方式。
