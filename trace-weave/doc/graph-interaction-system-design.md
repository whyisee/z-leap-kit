...
> 状态：阶段 A～D 核心闭环已实现并完成数据库迁移、自动化测试与浏览器回归
> 日期：2026-08-12
...
- 统一的 `resolve / execute` 动作协议、五分钟交互上下文和幂等执行审计。
- 节点动态右键菜单，以及世界图用户、实体和地点的上下文动作解析。
- “我”节点拖拽碰撞、候选目标高亮、停留确认和松开后保留落点。
- 游戏、书籍、影视、歌曲、App、食物、地点等常见实体的快捷记录模板。
- 快捷记录复用既有 AI 解析与用户确认流程，并在关系图内完成确认或暂存，不绕过“确认后入账”、不打断图谱上下文。
- 用户节点关系状态解析、好友申请入口和已有会话入口。
- 桌面侧边操作面板、移动端底部操作面板、减少动画偏好和交互审计表。

本轮进一步实现了通用节点组合解析：人物/账号、个人实体、定位地点可以拖入属于用户自己的事件；两条自有事件可以建立延续、因果、重复、引用和同时发生关系；人物、地点、事物之间可以在图内生成组合记录。世界中的公开事件仍只用于生成用户自己的新记录，不能反向修改原作者数据。

完整收尾实现还包括：共同经历作为个人图谱节点并支持自有事件授权关联；同类个人实体可进入图鉴比较并在强确认后合并；关系边支持证据查看、聚焦、时间线和当前视图隐藏；Shift/键盘/详情按钮可选择 2～12 个节点生成组合记录；事件、实体与共同经历写入均生成十分钟撤销令牌；个人布局可保存，当前隐私过滤后的视图可导出并标注时间。所有跨领域写入仍由服务端重新解析节点、校验所有权并写入审计。
...
支持 Shift 多选人物、地点和事物，也支持从节点详情使用“加入组合”（触屏和键盘等价入口），随后通过顶部组合动作条生成记录草稿，例如：
...
- `collision-candidate`：目标节点出现微弱外环；接近阶段产生轻微磁吸，真正重叠后目标沿碰撞方向产生有限位移。
- `drop-ready`：目标节点出现明确高亮，并显示“松开以选择操作”；支持触感的设备给一次短促反馈。
- `invalid-target`：不弹窗，可使用短暂灰色反馈，不使用红色错误打断拖拽。
- `action-sheet`：双方执行一次 220～300 毫秒阻尼弹簧收尾，保留源节点最终落点并打开动作选择器。

不使用无限呼吸、粒子或持续力导动画。普通拖拽松开后也只运行一次短时惯性/阻尼动画；物理反馈限制在约 30fps，并尊重 `prefers-reduced-motion`。
...
- [已实现] 图谱交互控制器与 G6 事件适配。
- [已实现] 动作解析、执行、幂等和审计。
- [已实现] 动态节点菜单、本地动作与查询动作。
- [已实现] 服务端稳定节点引用解析和执行前重新授权。

### 阶段 B：核心碰撞闭环

- [已实现] 碰撞检测、停留反馈、释放动作卡和单次阻尼弹性反馈。
- [已实现] “我 → 用户”的认识界面并复用好友服务。
- [已实现] “我 → 游戏/书/影视/App/地点”的类别化快捷记录。
- [已实现] 快捷记录在关系图内完成 AI 候选确认并复用草稿上限。

### 阶段 C：个人图谱编辑动作

- [已实现] 人物/账号 → 自有事件的参与者修订。
- [已实现] 个人实体/定位地点 → 自有事件的实体与地点修订。
- [已实现] 自有事件 → 自有事件的事件关系。
- [已实现] 事件 → 共同经历的成员授权关联。
- [已实现] 同类个人实体碰撞进入图鉴比较；强确认后可合并，并可撤销。

### 阶段 D：组合与高级动作

- [已实现] 两节点及 2～12 节点组合记录，统一进入确认流程。
- [已实现] 关系边右键证据查看、聚焦、时间线与当前视图隐藏。
- [已实现] 个人布局本地保存、重置，以及当前隐私过滤视图导出。
- [已实现] 交互按可见节点工作、无无限动画、拖拽计算限帧；后续规模超过约 300 个可见节点时可无破坏切换空间索引与分片加载。
- [已实现] 可逆领域操作统一撤销栈：事件参与者/实体/地点/事件关系、共同经历关联、实体合并。
...
- 图谱动作核心闭环已经完成；后续新增动作只需注册新的节点组合和领域适配器，不需要改变图谱数据模型或推翻前后端架构。
