...
织络是一个会自动理解用户的全域 AI 生活流水账。用户只需要用自然语言讲述发生过的事情，系统就能把现实活动和数字活动统一整理成事件、实体与关系，生成生活回顾，并在用户明确授权的情况下发现具有共同经历或兴趣的人。
...
- **世界**：公共图谱读模型。展示全站活跃用户、被事件所有者明确设为公开的正式事件，以及公开事件关联的非敏感标准实体。关系模式聚合“用户—地点/事物”，事件模式保留“用户—公开事件—地点/事物”证据链。
- **我**：账户私有图谱。展示当前用户全部已确认事件、私人人物、地点、事物、声明、推断和授权关系，不要求事件公开。

世界图不使用匿名匹配结果代替公共数据，也不读取私人事件来填充网络。公开事件在世界图中只投影事件标题、活动类型、按天降精度的时间和安全标准实体；原始输入、附件、精确时间、精确坐标、私人别名及未确认人物身份不进入公共读模型。匿名关系发现仍使用独立的最小化社交投影。
...
- 即使事件被设为公开，也不向其他用户暴露原始文字、语音、附件、精确时间和精确位置；世界图只展示经过降精度和安全过滤的公开事件投影。
...
1. 所有正式事件默认只进入个人时间线和个人图谱；明确设为公开的事件另外进入世界图公共投影。
2. 用户主动开启“参与关系发现”后，符合条件的事件才有资格生成匿名社交投影；该授权与是否公开相互独立。
...
匿名发现形成的双向连接和用户主动发送好友申请形成的关系必须汇入同一份 `social_connections`，不能维护两套互不相认的“好友”。用户可以按用户名或昵称查找账号、发送带可选留言的申请；接收方可同意或拒绝，发送方可在处理前撤回。双方成为好友后才允许创建一对一会话和发送消息，解除关系或任一方拉黑后立即停止发送，但历史会话仍可由原成员查看。

私聊消息与生活流水属于不同的数据域：消息不会自动送入 AI、事件库、时间线或关系图谱，也不会因为聊天中提到地点和人物就生成生活事实。只有用户主动把某段经历重新作为生活记录提交时，才进入“解析—确认—入账”流程。

一对一消息域包含：

- `friend_requests`：发送方、接收方、留言和待处理/同意/拒绝/撤回状态。
- `direct_conversations`：一对用户唯一一条会话，保留最后消息时间。
- `direct_conversation_members`：成员的已读时间、隐藏和静音状态。
- `direct_messages`：发送者、文本、客户端幂等 ID、创建/编辑/删除状态。
- `notifications`：好友申请、申请通过和新消息提醒；打开会话后同步清除对应未读状态。

消息发送需要同时验证会话成员身份、当前好友关系和双向屏蔽状态，并实施长度限制、频率限制和客户端幂等。账号导出包含好友申请、连接、会话和消息；账号删除会移除申请和成员关系，并将该用户已发送消息替换为删除墓碑，避免继续保留其文本内容。
...
织络的目标形态是覆盖现实与数字生活的通用事件系统。任何活动都先被保存为可追溯的原始记录，再由通用事件信封、类型化实体关系和领域扩展结构化，最终形成时间线、查询、统计、个人图谱与隐私可控的社交能力。
...
## 16. 实现进度审计（2026-08-11）

本节以当前仓库中的前端页面、后端路由和服务、16 个数据库迁移、自动化测试及真实 DeepSeek/数据库冒烟为证据进行审计。仅创建了表、枚举或页面占位但没有完整业务入口、权限求值和生命周期处理的能力，不计为“已完成”。
...
| 世界关系图谱与个人图谱 | **大部分完成** | “世界”读取可撤销的公共事件/实体投影，展示全站活跃用户、明确公开事件和安全标准实体；Outbox 实时更新且每小时完整对账。关系/事件模式分别提供聚合网络与证据链；“我”保留账户完整私有图谱 | 数据规模增长后仍需游标分片、时间窗、节点聚类和视口增量加载；外部公共数据源尚未选型接入 |
...
| 好友与一对一私聊 | **已完成（应用层）** | 用户搜索、申请/同意/拒绝/撤回、统一好友关系、好友列表、会话、文字消息、未读/已读、轮询更新、解除/拉黑后禁发、通知与数据治理均已贯通 | 图片/文件消息、消息撤回和 WebSocket/SSE 属增强项，不影响当前一对一文字私聊闭环 |
...
| 异步任务与派生一致性 | **大部分完成** | Outbox、通知和删除消费者支持租约、跳过锁、幂等、指数退避、死信；公共世界图同时具备实时增量刷新、advisory lock 防重和每小时全量修复；指标暴露队列及投影状态 | 管理后台重放和外部告警接收器可后续增加 |
| 测试与可运维性 | **大部分完成** | 全量构建、前后端自动化测试；真实 DeepSeek、好友与私聊双账号接口链路、浏览器交互、实体拆分撤销、世界投影公开/撤销和数据库冒烟；CI、指标、备份校验、重启和负载冒烟齐备 | 自动化浏览器 E2E、隔离恢复演练和真实生产外部服务集成仍待部署环境完成 |
...
**当前状态：已完成应用层闭环（2026-08-11）。** 手动好友申请与匿名发现连接复用统一好友关系；好友列表、会话列表、文字消息、未读/已读、消息通知、发送幂等和频率限制均已实现。真实双账号链路已验证搜索、申请、同意、发信、未读归零，以及解除关系后返回 403 禁止继续发送。
